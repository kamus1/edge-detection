...
__global__ void kernelNaive(float *imgs, float *out, int L, int M, int N, float *kernel_1, float *kernel_2) {
...
int main(int argc, char **argv){
    ...
    // cudaEventCreate(&ct2);
    // cudaEventRecord(ct1);
    // kernelNaive<<<grid_size, block_size>>>(imgsDev, OutDev, L, M, N, kernel_1Dev, kernel_2Dev);
    // cudaEventRecord(ct2);
    // cudaEventSynchronize(ct2);
    // cudaEventElapsedTime(&dt, ct1, ct2);
    // cudaMemcpy(out, OutDev, L*M*N*sizeof(float), cudaMemcpyDeviceToHost);
	// printf("Tiempo GPU: %f[ms]\n", dt);
	// Write(OutDev, L, M, N, "out_dev.txt");

	// cudaFree(imgsDev); cudaFree(OutDev); cudaFree(kernel_1Dev); cudaFree(kernel_1Dev);
